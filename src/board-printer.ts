...
export function printBoard(
  ...
  const verticalWalls = board.tiles
    .map(row =>
      row
        .slice(0, -1)
        .reduce((str, cell) => str + (cell & Wall.RIGHT ? 1 : 0), '')
    )
    .join('')
    .split('')
    .map(v => v === '1');
  const horizontalWalls = board.tiles
    .slice(0, -1)
    .map(row => row.reduce((str, cell) => str + (cell & Wall.DOWN ? 1 : 0), ''))
    .join('')
    .split('')
    .map(v => v === '1');
  ...
  const imageHeight =
    imagePadding * 2 + height * (cellGap + cellSize) + cellGap;
  ...
  ctx.fillRect(
    0,
    0,
    width * (cellSize + cellGap) + cellGap,
    height * (cellSize + cellGap) + cellGap
  );
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width - 1; x++) {
      ...
      ctx.fillRect(
        (cellGap + cellSize) * (x + 1) + cellGap / 2 - wallThickness / 2,
        y * (cellSize + cellGap) + cellGap / 2,
        wallThickness,
        cellSize + cellGap
      );
    }
  ...
  for (let y = 0; y < height - 1; y++) {
    for (let x = 0; x < width; x++) {
      ...
      ctx.fillRect(
        x * (cellSize + cellGap) + cellGap / 2,
        (cellGap + cellSize) * (y + 1) + cellGap / 2 - wallThickness / 2,
        cellSize + cellGap,
        wallThickness
      );
    }
  ...
  ctx.translate(
    goal.x * (cellGap + cellSize) + cellGap,
    goal.y * (cellGap + cellSize) + cellGap
  );
  ctx.beginPath();
